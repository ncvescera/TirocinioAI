...
def stinson(in_image, intensity=1, alpha=1):
    bg_image = create_filled_image(in_image.size, [240, 149, 128, 0.2])
...
def slumber(in_image, intensity=1, alpha=1):
    bg_image_1 = create_filled_image(in_image.size, [69, 41, 12, 0.4])
    image = alpha_compositing(in_image.copy(), bg_image_1.copy(), lambda s, b: ImageChops.lighter(s, b))
    bg_image_2 = create_filled_image(image.size, [125, 105, 24, 0.5])
...
def perpetua(in_image, intensity=1, alpha=1):
    bg_image = linear_gradient(in_image.size, [230, 193, 61], [0, 91, 154], 'v')
    image = alpha_compositing(in_image.copy(), bg_image.copy(), soft_light_blending)
    image = Image.blend(in_image, image, 0.5 * alpha)
    return interpolate(pil_to_array(in_image), pil_to_array(image), intensity)
...
def rise(in_image, intensity=1, alpha=1):
    ...
    bg_image_1 = create_filled_image(in_image.size, [236, 205, 169, 0.15])
    ...
    bg_image_2 = create_filled_image(in_image.size, [50, 30, 7, 0.4])
    ...
    gradient_image_1 = radial_gradient(in_image.size, [255, 255, 255], [0, 0, 0]).convert('L')
    
    # unisce le tre immagini precedenti 
    image_1 = Image.composite(image_1, image_2, gradient_image_1)

    # applica colore rosa chiaro all'immagine prodotta in precedenza
    bg_image_3 = create_filled_image(in_image.size, [232, 197, 152, 0.8])
    ...
    gradient_image_2 = radial_gradient(in_image.size, [255, 255, 255], [0, 0, 0]).convert('L')
    image_3 = Image.composite(image_2, image_1, gradient_image_2)

    image = Image.blend(image_1, image_3, 0.6)
    image = adjust_brightness(image, 1.05 * alpha)
