...
def grayscale(image: str) -> str:
    ...
    # effettua la conversione a gray scale
    gray = ImageOps.grayscale(im)

    # trasforma l'immagine in stringa di byte
    # per essere inviata al classificatore
    buf = io.BytesIO()
...
def predict(model: str, img_path: str) -> list:
    ...
    res = requests.post(f'{url}/{model}', data=grayscale(img_path))
